...
import numpy as np
import matplotlib
matplotlib.use("Agg")  # Required for headless/parallel workers; avoids blank figures
...
from src.utils.result_paths import WFINSTANCES_SRASEARCH_RESULTS_DIR
...
OUTPUT_DIR = WFINSTANCES_SRASEARCH_RESULTS_DIR
...
# Fixed K mode: when True, uses k=n/2 instead of reversible jump MCMC
USE_FIXED_K = False  # SRASearch works well with reversible jump
...
def run_single_experiment(
    ...
    num_iterations: int,
    seed: int,
    noise_option: str = "frontier_softmax_uni_utility",  # Changed from log_successors_queue_jump
    use_fixed_k: bool = False,
) -> Dict[str, Any]:
    """
    ...
    # Determine K: fixed at n/2 or use reversible jump
    if use_fixed_k:
        fixed_K = n // 2
        print(f"  Running MCMC: {len(traces)} traces, {num_iterations:,} iterations, FIXED K={fixed_K}...")
    else:
        fixed_K = None
        print(f"  Running MCMC: {len(traces)} traces, {num_iterations:,} iterations, reversible jump K...")
    ...
    # Process-mining baselines (Inductive Miner, Heuristics Miner)
    task_ids = data['task_ids']
    scenario_id = f"srasearch_ip{ip_cov_target}"
    if PM4PY_AVAILABLE:
        try:
            im_cover = baseline_inductive_miner_cover(traces, task_ids, scenario_id=scenario_id, noise_threshold=0.0)
            p_im, r_im = precision_recall(true_cover, im_cover)
            out['inductive_miner_f1'] = f1_score(p_im, r_im)
        except Exception:
            out['inductive_miner_f1'] = None
        try:
            hm_cover = baseline_heuristics_miner_cover(traces, task_ids, scenario_id=scenario_id,
                dependency_threshold=0.5, and_threshold=0.65, loop_two_threshold=0.5)
            p_hm, r_hm = precision_recall(true_cover, hm_cover)
            out['heuristics_miner_f1'] = f1_score(p_hm, r_hm)
        except Exception:
            out['heuristics_miner_f1'] = None
    else:
        out['inductive_miner_f1'] = None
        out['heuristics_miner_f1'] = None
...
def run_single_ip_cov_experiment(
    ...
    try:
        ...
        k_mode = f"FIXED K={data['num_tasks']//2}" if use_fixed_k else "reversible jump"
        print(f"[IP-Cov={ip_target}] Traces: {len(traces)}, Realized: {realized_cov*100:.1f}%, K mode: {k_mode}")
        ...
        with PdfPages(diag_path) as pdf:
            ...
            axes[0].imshow(data['cover'], cmap='Blues', vmin=0, vmax=1, aspect='auto')
            axes[0].set_title(f"Ground Truth ({data['num_edges']} edges)")
            
            axes[1].imshow(result['inferred_cover'], cmap='Oranges', vmin=0, vmax=1, aspect='auto')
...
def main():
    ...
    parser.add_argument(
        "--fixed_k", action="store_true", default=USE_FIXED_K,
        help="Use fixed K=n/2 instead of reversible jump MCMC"
    )
    parser.add_argument(
        "--no_fixed_k", action="store_true",
        help="Use reversible jump MCMC for K (override --fixed_k)"
    )
    
    args = parser.parse_args()
    
    # Handle fixed_k flag
    use_fixed_k = args.fixed_k and not args.no_fixed_k
    ...
    print(f"K mode: {'FIXED K=n/2' if use_fixed_k else 'Reversible Jump MCMC'}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(
                ...
                i, ip_target, data, args.num_iterations, args.seed, output_dir, use_fixed_k
            ): ip_target
    ...
    print(f"\n{'IP-Cov':<10} {'Traces':<8} {'BHPOP F1':<10} {'AND F1':<10} {'Maj F1':<10} {'IM F1':<10} {'HM F1':<10} {'SHD':<6}")
    print("-" * 75)
    for r in all_results:
        if 'error' not in r:
            im_f1 = r.get('inductive_miner_f1')
            hm_f1 = r.get('heuristics_miner_f1')
            im_s = f"{im_f1:.3f}" if im_f1 is not None else "N/A"
            hm_s = f"{hm_f1:.3f}" if hm_f1 is not None else "N/A"
            print(f"{r.get('ip_cov_realized', 0):<10.2f} {r.get('num_traces', 0):<8} "
                  ...
                  f"{im_s:<10} {hm_s:<10} {r.get('bhpop_shd', 0):<6}")
        else:
    ...
    summary = {
        ...
        'config': {
            ...
            'use_fixed_k': use_fixed_k,
            'fixed_k_value': data['num_tasks'] // 2 if use_fixed_k else None,
        },
        'results': all_results,
    }
...
if __name__ == "__main__":
    ...
    # Fix for macOS: use spawn method to avoid fork issues with certain libraries
    try:
