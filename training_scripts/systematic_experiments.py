#!/usr/bin/env python3
"""
Systematic experiments for the Cloud-IaC paper benchmark.

Pipeline
1) Load manual scenarios (each scenario = one "assessor"; ground-truth DAG/poset cover)
2) Load traces from data/cloud_iac_dataset/expert_traces/ and data/cloud_iac_dataset/traces/ (optionally combine)
...
from src.utils.result_paths import CLOUD_IAC_RESULTS_DIR
...
# LIKELIHOOD ABLATION: To compare different likelihoods, modify this list:
# - log_successors_queue_jump: Q(a) = log(1 + S_t(a)) - prefers actions with more successors
# - frontier_softmax_uni_utility: Q(a) = 0 - uniform over frontier (no successor preference)
# - queue_jump: Nicholls-style NLE-based (expensive, #P-complete)
LIKELIHOODS = ["frontier_softmax_uni_utility"]  # New uniform utility likelihood only
...
# Output
OUTPUT_DIR = CLOUD_IAC_RESULTS_DIR
...
def load_manual_scenarios(path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Cloud-IaC scenario directory not found: {path}")
...
def augment_scenario_with_synthetic_traces(
    ...
    *,
    ...
    """Augment scenario with synthetic traces to reach target incomparable-pair coverage."""
...
def build_data_dict(project_root: Path) -> Dict[str, Any]:
    ...
    scenarios = load_manual_scenarios(scenario_root)
...
# =========================
def run_systematic_experiments(data: Dict[str, Any], start_exp: int = 1, end_exp: int = None) -> pd.DataFrame:
    """
    ...
    Args:
        start_exp: Start from this experiment ID (inclusive, 1-indexed)
        end_exp: End at this experiment ID (inclusive). None means run all.
    """
    rows: List[Dict[str, Any]] = []
    ...
    for cp_target in IP_COV_TARGETS:
        ...
        # --- per-scenario subsampling for this IP target ---
        for sid in scenario_ids:
    ...
    # =========================
    # Filter experiments if start_exp/end_exp specified
    # =========================
    if start_exp > 1 or end_exp is not None:
        original_count = len(experiments)
        if end_exp is None:
            end_exp = max(e["exp_id"] for e in experiments)
        experiments = [e for e in experiments if start_exp <= e["exp_id"] <= end_exp]
        print(f"\n📋 Filtering experiments: running {len(experiments)}/{original_count} (exp_id {start_exp}-{end_exp})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-exp", type=int, default=1, help="Start from experiment ID (inclusive)")
    parser.add_argument("--end-exp", type=int, default=None, help="End at experiment ID (inclusive)")
    parser.add_argument("--test", action="store_true", help="Run test mode")
    args = parser.parse_args()
    
    if args.test or (len(sys.argv) > 1 and sys.argv[1] == "test"):
    ...
    data = build_data_dict(PROJECT_ROOT)
    df = run_systematic_experiments(data, start_exp=args.start_exp, end_exp=args.end_exp)
    print(df.head())
