...
4) Control trace diversity using CP-Cov targets via subsampling (and optional synthetic augmentation)
...
def greedy_subset_indices_to_target(
    ...
    """
    Greedy select a subset of traces whose CP-Cov reaches `target` (as much as possible).
    ...
    if not pairs or target <= 0:
        # trivial: if no incomparable pairs, CP-Cov=1 automatically
...
def run_wfcommons_systematic(
    *,
    ...
    for group_name in group_names:
        ...
        print(f"traces={num_traces} unique={num_unique} base CP-Cov={base_cov:.3f}")

        # If we can't even do 1 trace, skip
        if num_traces == 0:
            continue

        # ---- Sweep CP targets (subsampling) ----
        for cp_target in cp_cov_targets:
            ...
            if not idxs:
                print(f"  CP target {cp_target}: no valid traces -> skip")
                continue
            ...
            print(f"  CP target {cp_target}: selected {len(idxs)} traces, realized CP-Cov={realized_cov:.3f}")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output_dir", type=str, default=str(PROJECT_ROOT / "notebooks/systematic_experiment_results_wfcommons"))
